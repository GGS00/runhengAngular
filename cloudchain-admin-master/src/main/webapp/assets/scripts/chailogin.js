/* Setup TmsController page controller */
angular.module('MetronicApp').controller('chailoginController', ['$rootScope', '$scope','$http','$location','uiGridConstants', 'settings','commonUtil','Table', function($rootScope, $scope, $http, $location,uiGridConstants,settings, commonUtil,Table) {

    $scope.$on('$viewContentLoaded', function() {
        App.initAjax(); // initialize core components
    });

    var vm = this;

    $scope.toSettled = function(){
        window.location.href="/settled"
    }

    $scope.chaimilogin = function(){
        var loginName = $("input[name='username']").val();
        var loginPwd = $("input[name='password']").val();

        var namereg = /^(?![0-9]*$)[a-zA-Z0-9]{4,20}$/;

        if (loginName == "" || loginPwd == "") {
            $('.adminalert').html('请输入用户名或密码')

            $('.adminDanger').show('normal');
            return false;
        }
        else {

            if(loginName.match(namereg) == null){
                $('.adminalert').html('用户名不合法')

                $('.adminDanger').show('normal');
                return false;
            }

            bodyRSA();
            var result = encryptedString(key, encodeURIComponent(loginPwd));
            $.ajax({
                type: "post",
                url: "/doLogin",
                data: {
                    uName:loginName,
                    uPass:result
                },
                dataType: "json",
                success: function(data){
                    if(data.status == '00'){
                        Cookies("loginUser", data.realName, { expires: 7 },{ path: '/' });
                        Cookies("loginPhoto", data.userPhoto, { expires: 7 },{ path: '/' });
                        window.location.href="/chaimihome";
                    }else{
                        $('.adminalert').html('登入失败'+data.msg)

                        $('.adminDanger').show('normal');
                        return false;
                    }
                }
            });

        }

    };

    var key;

    function bodyRSA(){
        setMaxDigits(130);
        key = new RSAKeyPair("10001","","");
    }

    $(document).keydown(function(event){
        if(event.keyCode == 13){
            $scope.chaimilogin();
        }
    });

}])



