angular.module('MetronicApp').controller('owneraddressCtrl', ['$rootScope', '$scope','$http','uiGridConstants', 'settings','suppServer','$stateParams','commonUtil','Table', function($rootScope, $scope, $http, uiGridConstants,settings, suppServer,$stateParams,commonUtil,Table) {
    ...
    vm.btaddAddress= function () {
        ...
        var isDefault = $('input[name="radio22"]:checked').val();
        vm.addrParams ={
    ...
    vm.updateAddress = function () {
        if(vm.addrEntity.getSelectedRows().length == 0){
            ...
            vm.upradioFlag =vm.addrEntity.getSelectedRows()[0].isDefault
        }
    }
    vm.btupdateAddress =function () {
        ...
        var isDefault = $('input[name="radio33"]:checked').val();
        vm.addrParams ={
