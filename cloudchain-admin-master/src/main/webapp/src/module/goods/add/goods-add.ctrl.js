 ...
 */
angular.module('MetronicApp').controller('GoodsAddController', function($rootScope, $scope, $timeout,Goods,commonUtil) {
    var editor_a = "";
        ...
        $scope.addImg = function () {
            var uploadFile = function (client) {
                ...
                return client.multipartUpload("cloudchain/gms/goods/images/"+file.name+"",file).then(function (res) {
                    var url = res.res.requestUrls[0].split('?')[0]
                    console.log(url)
                    if($scope.addType == 1){
                        $scope.$apply(function () {
                            vm.spuImgs.push({img:url});
                        });
                    }else{
                        $scope.$apply(function () {
                            vm.sku[index].skuImgs[0].img = url
                        })
                    }
