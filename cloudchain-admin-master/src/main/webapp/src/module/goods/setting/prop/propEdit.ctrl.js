angular.module('MetronicApp').controller('goodPropEditController',function($rootScope,$stateParams,$http,$scope,Goods) {
    ...
    var vm = this;
    vm.deleteList = []
    Goods.loadProp($stateParams.Id).success(function (data) {
        vm.propName = data.prop.propName;
        vm.propId = data.prop.propId
        vm.isApplyAll = data.prop.isApplyAll;
        var group = data.prop.groupList
        vm.propList = []
        for(key in group){
            var items = []
            for(key2 in group[key].paramList){
                var a = group[key].paramList[key2]
                items[key2] = {paramName:a.paramName,
            ...
            vm.propList[key] = {
                ...
                isCore:group[key].isCore,
                items:items
            }
        }
        var clist = []
        for(var i in data.prop.categoryList){
            clist.push(data.prop.categoryList[i].cId)
        }
    ...
    vm.rmItem = function (items) {
        var a = items.pop()
        if(a.paramId != "" && a.paramId != undefined){
            vm.deleteList.push({paramId:a.paramId})
        }
    ...
    vm.showAddValue = function (item) {
        ...
        if(vm.settingValue.values == undefined){
            vm.settingValue.values = [{val:""}]
        }

        vm.addSettingValue = function () {
            vm.settingValue.values.push({val:""})
    ...
    vm.addProp = function () {
        ...
        for(key in vm.propList){
            ...
            for(key2 in vm.propList[key].items){
                ...
                for(t in vm.propList[key].items[key2].values){
                    if(vm.propList[key].items[key2].values[t].val == ""){
                        msgAlert.text('有未填写的选择项')
                        return;
                    }
                    n.push(vm.propList[key].items[key2].values[t].val)
                }
                list.push({
        ...
        var info ={
            ...
            paramList:list,
            deleteParamList:vm.deleteList
        }
        console.log(info)
        Goods.updateProp(info).success(function (data) {
            if(data.status==00){
                window.history.back();
            }else{
