angular.module('MetronicApp').controller('addressCtrl', ['$rootScope', '$scope','$http','uiGridConstants', 'settings','suppServer','$stateParams','commonUtil','Table', function($rootScope, $scope, $http, uiGridConstants,settings, suppServer,$stateParams,commonUtil,Table) {
    ...
    vm.column_addr = [
        ...
        },
        {
            ...
            displayName: '地址类型',
            width: '20%',
            cellTemplate:'<div style="padding:5px" ng-model="address.addrType1[row.entity.index]">{{grid.appScope.$parent.address.addrType1[row.entity.index]}}</div>'
        },
        {
            name: "qty",
            displayName: '操作',
    ...
    vm.updateAddress = function () {
        if(vm.addrEntity.getSelectedRows().length == 0){
            ...
            var strs= new Array(); //定义一数组
            if(vm.addrEntity.getSelectedRows()[0].addrType==null){
                vm.addrEntity.getSelectedRows()[0].addrType ="";
            }
