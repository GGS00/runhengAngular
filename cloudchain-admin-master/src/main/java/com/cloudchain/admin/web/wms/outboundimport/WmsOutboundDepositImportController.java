 ...
 * 铺仓出库单（寄存）
 */
@Api(value = "wms-outbound-deposit-import-api", description = "出库单导入接口-铺仓出库单")
@Controller
@RequestMapping("/wmsOutboundDepositImport")
public class WmsOutboundDepositImportController extends ImpExpController<WmsOutboundRequest, WmsOutboundResponse> {
	private static final Logger logger = LoggerFactory.getLogger(WmsOutboundDepositImportController.class);
