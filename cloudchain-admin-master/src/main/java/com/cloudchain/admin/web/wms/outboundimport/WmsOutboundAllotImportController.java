 ...
 * 调拨出库单
 */
@Api(value = "wms-outbound-allot-import-api", description = "出库单导入接口-调拨出库单")
@Controller
@RequestMapping("/wmsOutboundAllotImport")
public class WmsOutboundAllotImportController extends ImpExpController<WmsOutboundRequest, WmsOutboundResponse> {
	private static final Logger logger = LoggerFactory.getLogger(WmsOutboundAllotImportController.class);
	...
	protected String[] getImpHead() {
		return new String[]{"货主编号", "单据类型", "加急/不加急", "出货仓库编号", "联系人", "联系电话", "省", "市", "区", "详细地址", "描述",
				"供应商", "商品编号", "数量", "良品/不良品", };
	}
